...
MaterialData *GetMaterialData(MaterialHandle handle, const char *functionName) {
    MaterialData *data = ResourceSystem::Get().Get(handle);
    if (!data) {
...
ShaderHandle MaterialGetShaderHandle(MaterialHandle handle) {
    MaterialData *materialData =
        GetMaterialData(handle, "MaterialGetShaderHandle");
    if (!materialData)
        return {};
...
void DestroyMaterial(MaterialHandle handle) {
    if (!ResourceSystem::Get().Get(handle))
        return;
    ResourceSystem::Get().Destroy(handle);
}

void MaterialSetFloat(MaterialHandle handle, const std::string &name,
                      float value) {
...
void MaterialSetVec3(MaterialHandle handle, const std::string &name,
...
void MaterialSetVec4(MaterialHandle handle, const std::string &name,
...
void MaterialSetTexture(MaterialHandle handle, const std::string &name,
...
void MaterialSetMat4(MaterialHandle handle, const std::string &name,
