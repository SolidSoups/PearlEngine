...
bool ParseObjFile(const char *path, std::vector<unsigned int> &outObjIndices,
...
/**
 * Reformats the parsed obj format to a format suitable for OpenGl. Vertices
...
MeshOldHandle CreateMesh(const std::vector<Vertex> &vertices,
    ...
    glBindBuffer(GL_ARRAY_BUFFER,
                 VBO); // bind the VBO to the target GL_ARRAY_BUFFER
    glBufferData(GL_ARRAY_BUFFER, vertices.size() * 8 * sizeof(float),
                 vertices.data(),
                 GL_STATIC_DRAW); // feed the target of GL_ARRAY_BUFFER (which
                                  // is our VBO) the data for the vertices

    // only generate Element buffer object if we need it...
    if (indices.size() > 0) {
        glGenBuffers(1, &EBO); // generate a buffer for EBO
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,
                     EBO); // bind the buffer to target GL_ELEMENT_ARRAY_BUFFER
        glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                     indices.size() * sizeof(unsigned int), indices.data(),
                     GL_STATIC_DRAW); // feed the buffer some data (mmmmmmmm)
    }
