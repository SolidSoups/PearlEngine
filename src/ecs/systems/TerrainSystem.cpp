...
void TerrainSystem::generateValidTerrains() {
  for (ecs::Entity e : Entities) {
    ...
    if (terrain.mesh && !terrain.isDirty)
      continue;
...
void TerrainSystem::generateTerrain(const TransformComponent &aTransform,
  ...
  generateVertices(*aTerrain.pixelCache, vertices, aTerrain.resolution);
...
float sampleHeightMap(const StbiImage &heightMap, glm::ivec2 coord) {
  coord = glm::clamp(coord, glm::ivec2(0),
                     glm::ivec2(heightMap.width - 1, heightMap.height - 1));
  return heightMap.data[coord.x + coord.y * heightMap.width] / 255.0f;
}

bool TerrainSystem::calculateTerrainHeight(
    const TerrainComponent &aTerrain,
    const TransformComponent &aTerrainTransform,
    const glm::vec3 &samplePosition, float &outHeight) {
  // no pixel cache
  ...
  // convert sample position to [u, v] coordinate relative to 
  // the terrain
  float terrainMinX = aTerrainTransform.position.x;
  float terrainMinZ = aTerrainTransform.position.z;
  float terrainMaxX = aTerrainTransform.position.x + aTerrainTransform.scale.x;
  float terrainMaxZ = aTerrainTransform.position.z + aTerrainTransform.scale.z;
  if(terrainMinX > terrainMaxX) std::swap(terrainMinX, terrainMaxX);
  if(terrainMinZ > terrainMaxZ) std::swap(terrainMinZ, terrainMaxZ);

  glm::vec2 uv(
    (samplePosition.x - terrainMinX) / (terrainMaxX - terrainMinX),
    (samplePosition.z - terrainMinZ) / (terrainMaxZ - terrainMinZ)
  );

  // skip if outside 0-1 range
  // non-exclusive check on 1.0 means the pixel size can never be out of bounds
  if(uv.x < 0.0f or uv.x >= 1.0f or uv.y < 0.0f or uv.y >= 1.0f)
    return false;

  // convert uv to pixel coordinates
  glm::ivec2 pixelPos(
    uv.x * aTerrain.pixelCache->width ,
    uv.y * aTerrain.pixelCache->height 
  );

  outHeight = sampleHeightMap(*aTerrain.pixelCache, pixelPos) * aTerrainTransform.scale.y;
  return true;
}


// get the normal of a coordinate using pixel coordinates
glm::vec3 calculateHeightMapNormal(const StbiImage &heightMap, glm::ivec2 coord,
                                   glm::ivec2 cellSize) {
...
void TerrainSystem::generateVertices(const StbiImage &heightMap,
                                     std::vector<float> &outVertices,
                                     size_t resolution) {
  outVertices.clear();
  outVertices.reserve(resolution * resolution *
                      8); // 9 floats (pos3, uv2, normal3) per cell
  ...
  for (int y = 0; y < resolution; y++) {
    for (int x = 0; x < resolution; x++) {
      ...
      glm::vec2 vertexUV((float)px / heightMap.width,
                         (float)py / heightMap.height);
      glm::vec3 vertexPos(vertexUV.x, heightSample, vertexUV.y);
      glm::vec3 vertexNormal = calculateHeightMapNormal(
          heightMap, glm::ivec2(px, py), glm::ivec2(widthStep, heightStep));
...
void TerrainSystem::generateIndices(std::vector<unsigned int> &outIndices,
                                    size_t columns, size_t rows) {
  outIndices.clear();
  outIndices.reserve((columns - 1) * (rows - 1) *
                     6); // 6 indices (2 triangles) per face
