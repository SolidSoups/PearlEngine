...
const glm::vec2 &InputManager::GetMouseDelta() const { return mouseDelta; }

const glm::vec2 &InputManager::GetMousePosition() const { return lastMousePos; }

bool InputManager::GetViewportMousePosition(glm::vec2 &mousePos) const {
  // calculate position relative to viewport
  glm::vec2 mouseViewport = lastMousePos - viewportMinPos;

  // ensure mouse is within viewport
  if (mouseViewport.x < 0 or mouseViewport.y < 0 or
      mouseViewport.x >= viewportSize.x or mouseViewport.y >= viewportSize.y)
    return false;

  mousePos = mouseViewport;
  return true;
}

float InputManager::GetScrollDelta() const { return scrollDelta; }
...
bool InputManager::GetKey(int key) const { return keyState[key]; }
...
bool InputManager::GetMouseKey(int key) const { return mouseKeyState[key]; }
