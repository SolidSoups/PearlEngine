  ...
  public:
    ...
    void RegisterLoader(const std::string &extension,
        ...
        m_Loaders.insert_or_assign(
            extension,
            LoaderEntry{std::move(loader), std::type_index(typeid(AssetType))});
        LOG_INFO << "Registered loader for extension '" << extension << "' "
    ...
    AssetLoader<AssetType> *GetLoader(const std::string &extension) {
        ...
        if (it->second.assetType != std::type_index(typeid(AssetType))) {
            LOG_ERROR << "Type mismatch! Extension '" << extension
                      << "' is registered for type '"
                      << it->second.assetType.name() << "' but you requested '"
    ...
    // Load asset directly
    template <typename AssetType>
    std::optional<AssetType> Load(const pe::FileDescriptor *file) {
