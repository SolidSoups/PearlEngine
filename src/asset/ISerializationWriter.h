...
public:
  ...
  virtual void WriteData(const std::string &name, const unsigned char *data,
                         size_t size) = 0;

  // several values
  virtual void WriteFloats(const std::string &name,
                           const std::vector<float> &values) = 0;
  virtual void WriteInt32s(const std::string &name,
                           const std::vector<uint32_t> &values) = 0;

    public : virtual std::vector<uint8_t>
             GetBytes() const = 0;
};
