...
void MenuBar::SetButtonCallbacks(std::function<void()> &onPlayClicked,
...
void MenuBar::RenderMenuBar(RuntimeState runtimeState) {
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (runtimeState == RuntimeState::RUNTIME or runtimeState == RuntimeState::PAUSED) {
      ...
      if (ImGui::ImageButton("##stop", (ImTextureID)(intptr_t)mStopTex->id,
                             ImVec2(buttonSize, buttonSize)))
        if (mOnStopClicked)
          mOnStopClicked();
      ImGui::SameLine();
      ImGui::SetCursorPosY((windowHeight - buttonSize) * 0.5f);
      if (ImGui::ImageButton("##pause", (ImTextureID)(intptr_t)mPauseTex->id,
                             ImVec2(buttonSize, buttonSize)))
        if (mOnPauseClicked)
          mOnPauseClicked();
      ImGui::SameLine();
      ImGui::SetCursorPosY((windowHeight - buttonSize) * 0.5f);
      if (ImGui::ImageButton("##reload", (ImTextureID)(intptr_t)mReloadTex->id,
                             ImVec2(buttonSize, buttonSize)))
        if (mOnReloadClicked)
          mOnReloadClicked();
    } else {
      ...
      ImGui::SetCursorPosY((windowHeight - buttonSize) * 0.5f);
      if (ImGui::ImageButton("##play", (ImTextureID)(intptr_t)mPlayTex->id,
                             ImVec2(buttonSize, buttonSize)))
        if (mOnPlayClicked)
          mOnPlayClicked();
    }
